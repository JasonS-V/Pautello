...
/**
 * Calculates total beat duration of an item (accounting for dotted notes and tuplets)
 */
export function getItemBeats(
  duration: NoteDuration,
  isDotted?: boolean,
  tuplet?: TupletInfo | null
): number {
  const base = DURATION_BEATS[duration] || 1;
  const dotted = isDotted ? base * 1.5 : base;
  if (tuplet && tuplet.actual > 0) {
    return dotted * (tuplet.normal / tuplet.actual);
  }
  return dotted;
}
 ...
 */
export const KEY_SIGNATURE_DATA: Record<
  KeySignature,
  { accidentals: number; type: '#' | 'b' | 'none'; name: string }
> = {
